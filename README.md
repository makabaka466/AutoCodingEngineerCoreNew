...
问题描述 + 可选页面标题/路径 + 可选异常截图
  ...
  -> 模型结合对话、映射 URL、当前代码以及可用截图验证页面
     ...
     -> 定位页面、请求链路、服务与数据访问代码
        ...
        -> 主机自动校验并执行，不要求用户手工查询
        -> SQL 失败：脱敏错误自动返回模型，在独立纠错额度内自行修正
...
页面定位最多两轮、业务数据最多两轮、SQL 纠错一轮、每条默认最多 100 行，以及
password/token/secret 等敏感列脱敏。接口已预留 `source` 与 `external_reference`，后续钉钉
机器人可以直接创建和继续同一异常会话。审计只保存查询阶段、成功/失败状态、脱敏错误、SQL
指纹、参数名、用途、行数和脱敏信息，
不保存参数值与原始业务行。结果数量不明确时，模型会先使用 100 条有界采样；能用更少数据
完成判断时仍优先采用更小限制。

模型查询契约优先使用 `:name` 占位符，参数字典键写 `name`。SQL Server 宿主同时安全兼容模型
常见的 `@name` 写法：两者都会转换成 pyodbc 的 `?` 并单独绑定值，绝不把值拼接到 SQL 中。
