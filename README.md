...
- 每个完成工作轮次自动生成一份独立的可复用能力文档，供这个项目后续任务按需参考。
     ...
     -> 清楚：读取目标及必要关联代码
        ...
        -> 需要修改：先展示现状、修改方案、目标效果、影响、验证计划和可用预览
           -> 用户确认方案后：请求并使用 modify 授权实施
              ...
              -> 验证并完成
  -> 保存任务会话
  -> 完成时写入本轮独立能力文档和 CAPABILITIES.md 索引
  -> 用户继续追问：同一会话开启下一工作轮次，重新分析并在完成时写入新的 MD
```
...
能力文档仍按目标工作区、按流程和工作轮次分开保存，不会把后续完成内容追加进旧 MD。
...
决策和产物，同时重置本轮查询/重规划预算。每次再次完成都会生成新的 cycle 能力文档。
