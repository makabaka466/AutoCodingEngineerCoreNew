...
形成结构化查询；宿主直接执行查询并负责 SQL Server 只读连接、SQL 写操作拦截、5 秒查询超时、最多两轮查询、每条
最多 50 行，以及
...
客户端采用面向长任务的浅色 AI 工程工作台：左侧约 260 px 的任务导航、中部低干扰对话阅读区、
底部固定任务上下文输入区，以及只使用一个高强调主操作的按钮层级。顶部胶囊按钮可明确选择
