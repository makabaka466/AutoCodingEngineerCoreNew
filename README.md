...
```text
问题描述 + 页面名称，或包含页面标题的异常截图
  -> 模型先确认页面名称
     -> 截图存在：优先识别窗口/标签页/窗体/页面主标题，不扫描全部页面映射
     -> 标题不可靠：只追问页面名称或要求包含标题的截图
     -> 标题可靠：按项目知识先做有界精确/前缀映射，无结果再做有界关键词模糊映射
  -> 模型结合名称、映射 URL、当前代码以及可用截图验证页面
     -> 定位页面、请求链路、服务与数据访问代码
...
通用异常调查规则作为应用内置的异常专用 Markdown 每轮强制加载；项目表结构、页面映射方式和
