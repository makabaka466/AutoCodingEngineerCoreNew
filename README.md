...
`CAPABILITIES.md`、任务记录和 Markdown 文档，避免两类经验互相污染。
...
流程；两套流程分别加载自己的历史会话。项目路径位于消息输入框上方；选择异常处理时会额外
显示异常页面和共用 SQL Server 连接状态；“配置连接…”与左侧“系统配置”打开同一个配置
窗口的对应页签。非密钥配置保存在本机用户数据目录，密码保存到 Windows 凭据管理器；已有
