...
本文描述当前 `0.7.13` 代码已经实现的架构。数据字段、公共方法和命令行参数见
...
项目 `CLAUDE.md` 不自动注入，模型只在任务相关时把它作为普通、不可信项目上下文主动读取。

源码搜索另有共享的有界策略：模型根据页面映射、类名、路由和当前证据选择搜索目标，宿主只阻止
确定性的越界/失控行为。生产可观测 Runtime 单轮最多接受 8 次 `Glob/Grep`，拒绝 `**/*`、
`**/*.cs` 等全库模式；目录级 `Grep` 必须带 `glob/type` 和 1..100 的 `head_limit`，路径只能位于
工作区或主机明确授权的只读目录。拦截会写入 `policy_blocked` Runtime 活动并终止本轮，避免继续
扩大读取范围。对于缺少 `glob/type/head_limit` 或首个过宽 Glob 等可纠正的模型参数错误，开发与
异常的 inspect 流程会把脱敏原因返回同一个 Runtime Session，自动允许一次更窄的只读搜索；原
调用结果不会被采纳，并追加 `policy_repair_requested`。第二次仍违规、路径越界或搜索调用预算耗尽
时直接失败，不能靠重试扩大边界。精确模式（如 `**/FCModelUpload.cs`）仍可跨工作区内的嵌套
解决方案定位源码。

异常流程还在工具白名单上实施阶段门控。当前 cycle 没有非空页面映射候选时只开放 `Read`，模型
必须先返回 `page_lookup` 查询计划，由宿主查询配置数据库；精确查询 0 行时下一轮仍不开放搜索，
允许模型继续一次有界模糊映射。只有本 cycle 的成功页面查询返回至少一行候选，或页面已验证并
完成业务数据查询后，才开放 `Glob/Grep` 核对当前源码。这样“先查 Menu、再查源码”不再只依赖
Prompt 自律，也不会让一个过早的宽泛 Glob 直接把异常会话置为失败。
...
异常查询由模型显式标注为 `page_lookup` 或 `business_data`。前者用于最多两轮有界页面映射，后者
只有在页面源码已验证后才能使用，并拥有独立的两轮业务数据额度；SQL 执行错误另有一次纠错额度，
...
`:name`，宿主也兼容 SQL Server 常见的 `@name`，两者都转换为 ODBC `?` 参数并单独绑定值，绝不
拼接用户值；字符串字面量、引用标识符和 `@@` 系统变量不会被误替换。游标设置查询超时，返回行数取模型请求与主机上限的
...
阻止编辑，不阻止用户取得自己的对话内容。`sel` 标签被提升到所有消息卡片背景标签之上，正文与
无背景的发送者标题使用同样可见的选区颜色。
...
源码搜索纠正预算按“连续违规链”而不是整条用户命令计算。可纠正的只读搜索被阻断后，Engine
允许模型补齐一次 `glob/type/head_limit`；一旦收到通过 Runtime 校验的结构化决策，该违规链即
结束。后续页面定位、源码追踪或数据库回灌阶段若出现新的独立漏参，可以再获得一次纠正机会。

连续第二次违反搜索策略仍会终止当前命令，路径越界和预算超限也始终不可纠正。这个范围既避免
长调查因不同阶段各一次参数遗漏而提前失败，也不允许模型在同一违规点无限试探安全边界。开发
与异常流程复用相同语义，但只读 `inspect` 之外的实现、验证阶段不自动重试。

## 14. 异常附件重封装与最终结论

桌面粘贴的截图先由 `IncidentAttachmentStore` 标准化为隔离目录中的 PNG。发送前再次校验路径必须
严格等于 `<attachment-root>/<id>/incident-screenshot.png`，并验证文件存在、大小上限、像素上限、
PNG 格式及可解码性。若图片仍有效但文件大小元数据发生变化，只刷新 `size_bytes` 后交给 Engine；
损坏、越界或目录外文件仍被拒绝。Engine 保留原有严格校验，形成 UI 重封装和核心校验两层边界。

异常完成结果仍由模型判断内容，但宿主固定组织用户可见结构：`结论`、`为什么出现这个异常`、
`解决方法`、`结论置信度`。Prompt 要求模型区分已确认根因、高概率原因和待验证假设；若模型没有
给出安全动作，宿主展示继续补证据的保守提示，不伪造修复方案。桌面后台异常与附件准备失败均
进入本地应用日志，便于继续对话失败时追溯模型启动前的宿主错误。

## 15. 分层上下文与完成后续聊路由

开发和异常流程不再在每个 inspect 模型轮次注入完整数据库目录。宿主只提供数据库类型、只读
边界和按需元数据查询方式；模型先从已验证源码提取候选表和字段，确实需要时再把小型元数据查询
与相关业务证据查询合并为同一批结构化 `SELECT/WITH`。原有参数化、60 秒超时、默认 100 行、
敏感列脱敏和查询轮次预算均保持不变。

异常会话完成后继续对话会先进入无工具的 `IncidentContinuationDecision` 路由。路由只接收上个
cycle 的页面、结论、诊断、动作、置信度和最多十条查询摘要，不接收原始数据库行，也不恢复旧
Claude CLI 对话。若现有证据足以回答，宿主直接完成新 cycle 并追加同一能力文档；若出现新症状、
新环境、截图或需要核对当前事实，路由返回 `investigate`，Engine 再恢复原完整 Runtime 会话、
页面定位、RAG、源码读取和数据库查询能力。
