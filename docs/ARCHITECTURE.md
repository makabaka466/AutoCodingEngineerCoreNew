...
本文描述当前 `0.2.0` 代码已经实现的架构。数据字段、公共方法和命令行参数见
[接口与数据契约](INTERFACES.md)。

## 1. 项目目标

AutoCoding Engineer 是一个平台无关的软件开发 Agent 内核。CLI 和 Streamlit 只是交互
入口，任务理解、代码调查、修改方案和完成判断交给 Claude Code 中的模型；Python 主机
负责不能交给模型自行决定的边界：
...
当前项目不是工作流平台，也没有 HTTP API、多 Agent 编排、插件市场、Git 发布流程或
后台任务队列。它聚焦于一个 Agent 从澄清、调查、申请权限，到修改、验证和总结的核心
闭环。
...
Skills 会在每一轮作为系统提示词的一部分显式加载。目前捆绑五种工作方法：
...
状态图表达正常使用路径；结构化契约本身允许模型在任意模式返回四种状态。只有
...
- `modify`：用户批准后，下一轮以 `implement` 模式执行；
- `verify`：用户批准后，下一轮以 `verify` 模式执行。

`approve()` 只批准当前待处理请求的范围。`reject()` 会把拒绝信息作为新消息，以
...
`ClaudeCodeRuntime` 是 `AgentRuntime` 的默认实现。它不重写模型的 Agent 循环，而是把
...
- 从 `AgentDecision` 生成的 JSON Schema；
- 工作区能力目录；
...
`session_id`。它将 `structured_output` 校验为 `AgentDecision`，并提取 token、费用、耗时和
turn 数量。CLI stderr/stdout 中常见的认证信息会在形成用户可见错误前脱敏。

## 6. 能力文档生命周期

能力记忆位于 `Settings.data_dir/workspaces/<workspace_id>/`，不会写入目标仓库。
`workspace_id` 是规范化工作区绝对路径（不区分大小写）计算出的 SHA-256 前 16 位，因此
不同路径的项目默认隔离。

```mermaid
flowchart LR
    PREPARE["每轮 prepare 工作区记忆目录"] --> READ["模型可读取 CAPABILITIES.md 和相关条目"]
    READ --> WORK["调查 / 修改 / 验证"]
...
能力 Markdown 包含适用场景、方法、验证、风险、任务证据和来源任务；task JSON 保存目标、
...
只读 `inspect` 轮次通过 Claude Code 的 `--add-dir` 获得该工作区能力目录，并被提示先查看
...
CLI 与 UI 只依赖 `AgentApplication`。新的交付媒介可以复用同一门面，无需复制会话、审批
或 Runtime 逻辑。模型运行时和会话存储分别通过 `AgentRuntime`、`SessionStore` Protocol
替换；当前能力存储由 `AgentEngine` 直接依赖具体的 `CapabilityStore`。
