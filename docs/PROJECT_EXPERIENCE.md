...
> 文档基线：2026-09-01，项目版本 `0.7.14`。本文以当前代码为准，并明确区分“已实现”、
...
| `AUTO_CODING_DATABASE_MAX_QUERY_ROUNDS` | 开发任务查询轮数上限 |
| `AUTO_CODING_INCIDENT_MAX_PAGE_QUERY_ROUNDS` | 异常页面定位成功查询轮数上限 |
| `AUTO_CODING_INCIDENT_MAX_BUSINESS_QUERY_ROUNDS` | 异常业务数据成功查询轮数上限 |
| `AUTO_CODING_INCIDENT_MAX_QUERY_REPAIR_ROUNDS` | 异常SQL纠错轮数上限 |
...
- `v0.7.5`：恢复原生 Glob/Grep，增加精确文件定位提示、搜索预算、范围校验和审计熔断；
- `v0.7.6`：异常流程先查页面映射，非空候选返回后才开放源码搜索，并补全阻断参数审计；
- `v0.7.7`：同 cycle 自动恢复模型漏传的已验证页面，并让桌面对话文本完整可选、可复制；
- `v0.7.8`：可纠正的源码搜索阻断自动进行一次有界修正，并修复正文选区颜色层级；
- `v0.7.9`：纠正预算改为按连续违规链计算，并以真实 MES 异常流程验证完整闭环；
- `v0.7.10`：修复继续对话的附件元数据漂移，并保证异常完成结果包含原因和解决方法；
- `v0.7.11`：引入紧凑续聊路由、按需数据库结构和查询批处理，降低上下文与空转轮次；
- `v0.7.12`：收敛双 Engine 的 Runtime/RAG 基础能力和双 Store 的 SQLite 生命周期基础设施，并补全主流程注释；
- `v0.7.13`：将双 Engine 与共享基础设施的维护注释统一为中文，明确代码注释与运行契约的语言边界；
- `v0.7.14（当前）`：形成“模型自适应选择行动、宿主固定证据完整性”的分阶段架构计划；本版本只交付计划，运行逻辑尚未切换；
...
SELECT/WITH 校验、参数绑定、限时、限行、脱敏和数据源引用检查；当前版本中，失败尝试计入独立
纠错上限，不再挤占页面定位或业务数据的成功查询额度。
...
## 35. 异常查询协议兼容与分阶段预算（v0.7.4）

### 35.1 问题背景与实际证据

一次“小米良率上传”异常测试在页面定位阶段结束，并提示查询超过两轮。Claude会话证据显示：第一轮
生成了SQL Server常见的`@page_name/@page_prefix`参数，但旧宿主只识别ACE约定的
`:page_name/:page_prefix`，因此查询在进入数据库前以`Unused query parameters`失败；第二轮改成
冒号格式后真实执行，但用户标题“小米良率上传”与菜单中的“小米良率数据上传”不完全一致，返回
0行；模型第三轮已经形成`%良率%`的有界模糊查询，却被统一两轮上限拒绝。只读复核第三条查询得到
5个候选，其中包含`小米良率数据上传 -> Ckhy.MES.Client.CKClient.CustomYieldUpLoad`。

因此问题不在业务数据表，也不是模型任意选择了某张表：流程尚处于`Menu`页面定位，业务数据查询
根本没有开始。根因由两个独立设计缺口叠加形成：参数占位契约过于脆弱，以及页面定位和业务数据
共用一个无法容纳“精确→模糊→业务”正常路径的总轮次预算。

### 35.2 方案权衡与最终选择

参数协议仍明确要求模型优先输出`:name`，因为它与SQLite兼容并保持ACE跨数据库契约一致；宿主
同时安全接受SQL Server模型常输出的`@name`。两种格式都只会被转换为pyodbc的`?`，值从结构化
`parameters`字典按出现顺序单独绑定，绝不做字符串替换或拼接。宿主识别时跳过字符串字面量、引用
标识符和`@@ROWCOUNT`等系统变量。

查询预算没有简单从2提高到4或5。固定总数虽然改动小，却仍会让无效页面候选、SQL语法纠错和业务
取证互相争抢额度，也无法解释到底在哪个阶段耗尽。本次改为模型选择语义阶段、宿主只做确定性
校验：

- `page_lookup`：最多两轮成功查询，覆盖精确/前缀及必要时的有界模糊查询；
- `business_data`：页面及源码已验证后才能进入，独立拥有两轮成功查询；
- SQL correction：默认允许一轮失败后的自动纠错，不计入上述成功轮次；
- `query_rounds`继续记录所有数据库执行尝试，便于兼容旧审计和衡量真实成本。

阶段由模型根据调查目的判断，宿主不通过表名、SQL关键词或文件名猜测；但
`business_data`缺少已验证`page`时由Pydantic契约确定性拒绝，防止绕过页面优先原则。

### 35.3 核心实现

- `IncidentDecision`增加`query_stage`；`IncidentSession`增加页面、业务和纠错三个计数器；完成会话
  续聊进入新cycle时重置计数器，但历史Event和QueryObservation不删除；
- `IncidentEngine`按阶段检查预算，成功完成整组查询后才增加对应成功轮次；失败把脱敏错误返回同一
  模型会话，在纠错额度内继续；
- `QueryObservation`增加`status/stage/error`。多查询计划中如果前几条成功、后一条失败，审计分别
  保存已成功项和真实失败项，不把未执行查询伪装成失败；原始SQL、参数值和业务行仍不持久化；
- 桌面对话的“数据查询”区域显示“页面定位/业务数据”、成功行数或失败摘要，用户无需再翻Claude
  transcript才能判断问题发生在哪一层；
- 通用异常Prompt和“生物”项目知识明确`:name`格式及不带前缀的参数键，`@name`只作为宿主防御性
  兼容，不成为新的首选契约。

### 35.4 技术难点与边界

**兼容不能退化为SQL插值。** 直接把`@name`替换成参数值最简单，但会引入注入、类型和转义问题。
正确边界是只解析占位符名称，最终仍交给ODBC参数绑定。

**失败次数与成功调查额度不是同一概念。** SQL传输或列名错误反映的是计划可执行性；页面精确/模糊
及业务取证反映的是调查深度。拆分后既允许模型修正偶发错误，也能确定性阻止无限循环。

**审计摘要必须准确且最小。** 只显示最终成功会隐藏第一次参数失败；保存完整SQL和参数又会扩大敏感
数据暴露。最终只持久化阶段、状态、脱敏错误、SQL指纹和参数名，并在UI呈现足够追溯的信息。

数据库能力没有扩大：仍只接受单条`SELECT/WITH`，保持`ApplicationIntent=ReadOnly`、60秒超时、
默认100行、敏感列脱敏和会话数据源绑定；生产账号仍必须只有最小读取权限。

### 35.5 决策记录

- ADR-103：ACE查询契约首选`:name`，SQL Server宿主防御性兼容`@name`，两者统一参数化绑定；
- ADR-104：异常查询按页面定位、业务数据和SQL纠错分配独立预算，不使用一个固定总轮次控制语义
  不同的调查阶段；
- ADR-105：失败查询进入QueryObservation和桌面摘要，但只保存脱敏错误与查询指纹，不保存SQL值或
  业务行。
...
## 37. 异常页面映射优先与源码搜索阶段门控（v0.7.6）

### 37.1 现象与日志证据

用户输入“小米良率上传页面，上传成功但日志没有保存”，首轮Runtime在读取异常能力文档后立即
尝试宽泛Glob，并被v0.7.5的有界搜索策略阻断。应用日志明确记录该会话在首次inspect run、尚未
产生任何`DATABASE_QUERIES_EXECUTED`事件时触发`search_policy_blocked`；SQLite事件顺序同样是
`system_init -> Read capability -> policy_blocked -> runtime_failed`。因此此次失败不是Menu查询没
找到页面，也不是业务数据查询错误，而是模型跳过了应有的页面映射阶段。

这也暴露了审计缺口：旧`policy_blocked`事件只保存原因，没有保存被拒绝的pattern/path，事后只能
确认“过宽”，不能精确判断模型用了`**/*`还是其他等价模式。

### 37.2 根因与方案选择

v0.7.5已经在Prompt里要求先利用页面映射，但首轮仍同时暴露Read/Glob/Grep。工具可见意味着模型
可以在没有数据库证据时自行选择仓库发现；熔断只能阻止后果，不能保证调查顺序。单纯放宽Glob会
让这次请求继续，但会退回“靠全局搜索猜页面”的低效路径，也违背生物项目已经提供的Menu映射。

本次将语义判断和确定性流程约束重新分层：模型继续判断用户话语中的页面标题、生成精确或模糊
Menu查询、比较候选；宿主只根据已经发生的结构化事实决定工具阶段。没有使用“小米”“良率”或
特定表名等业务关键词规则。
...
```text
用户问题/截图
  -> 首轮只开放 Read（图片、能力文档、用户给出的精确文件）
  -> 模型返回 page_lookup 参数化查询
  -> 宿主执行 Menu 精确/前缀查询
       -> 0 行：仍只开放 Read，允许一次有界模糊 page_lookup
       -> 非空候选：开放 Read + Glob + Grep
  -> 根据 URL/类名推导精确文件名并核对页面源码
  -> 从页面代码提取真实查询语义
  -> 宿主执行 business_data 只读查询
  -> 结合页面、代码和数据形成原因与方案
```

`_source_search_enabled()`只查看当前cycle的QueryObservation：成功`page_lookup`必须
`returned_rows > 0`才解锁；成功`business_data`也会保持解锁，因为该阶段的Pydantic契约要求已经
携带验证后的page。新cycle从自己的observation offset重新判断，不会让历史页面查询自动扩大新
问题的搜索权限。

### 37.4 审计增强与验证

拦截事件现在除reason外，还通过现有脱敏函数保存pattern、path、glob/type、output mode和
head_limit；源码匹配正文仍不持久化。定向测试验证工具序列为：首轮Read，精确映射0行后仍为Read，
模糊映射返回候选后为Read/Glob/Grep，业务数据轮后继续保持源码搜索。

真实Claude Code + DeepSeek smoke test使用同一MES工作区、当前SQL Server schema和“生物”异常
知识，只开放Read且不执行模型生成的业务查询。模型返回`query_required / page_lookup /`
`menu_lookup_yield_upload`，实际工具序列只有Read，证明首轮已从“扫描仓库”改为“请求宿主查页面
映射”。全量199项非live测试和Ruff通过；测试未写数据库、未修改MES代码、未调用上传接口。

### 37.5 决策记录

- ADR-110：异常新cycle在没有当前页面映射候选前只开放Read，不同时暴露Glob/Grep；
- ADR-111：成功但0行的页面查询不解锁源码搜索，继续由模型生成有界模糊映射；
- ADR-112：当前cycle存在非空页面候选后才开放原生搜索，语义候选选择仍由模型负责；
- ADR-113：policy阻断必须保存脱敏工具参数摘要，避免只有笼统原因而无法追溯。

---

## 38. 结构化页面连续性与可复制对话（v0.7.7）

### 38.1 失败现象与证据定位

一次真实异常调查已经完成 Menu 精确/模糊映射、用精确 Glob 找到页面源码、追踪日志写入调用链，
并成功执行第一轮 `business_data` 查询。随后模型认为还需要一轮业务证据，返回
`query_required / business_data`，但结构化 JSON 没有再次复制 `page`。旧 Pydantic validator 在
Runtime 解析边界立即抛出 `page is required before requesting business_data queries`，Engine 尚未
取得 `IncidentDecision`，因此无法利用前一轮已经存在的页面事实，整个任务被标为 failed。

SQLite Event/Run 与应用日志共同证明：页面定位和业务查询都已成功，失败点不是 SQL Server、Menu
查询或源码搜索，而是跨轮结构化输出重复字段遗漏。这个案例说明“强契约”与“对话连续性”不能只
靠模型每轮完美复述；宿主需要能区分可由既有事实确定性修复的遗漏，和真正缺少前置证据的越权。

### 38.2 方案权衡

没有删除业务查询的页面前置条件，也没有简单接受任意缺页决定。最终采用三段式边界：

1. `IncidentDecision` 的解析层允许缺少重复 `page`，使 Engine 有机会读取结构化决定；
2. `IncidentSession.located_page` 只保存本 cycle 已通过路径校验且包含源码路径的页面；
3. Engine 在 `business_data/completed` 缺页时只复用这个对象，并写入 `decision_repaired`；如果没有
   可复用对象、路径为空或越界，仍由宿主校验拒绝。

相比再次调用模型，这种恢复不增加 Token、延迟或新的语义漂移；相比把 page 变成完全可选，它又
保持了“业务数据必须绑定已验证页面”的安全约束。查询结果回灌还会显式携带已验证页面，降低模型
下一轮再次遗漏的概率。新 cycle 清空绑定，避免把历史页面静默用于另一项异常。

### 38.3 桌面对话可复制性

对话原本已经使用 disabled `tk.Text`，但箭头光标、默认选区和缺少显式复制入口让用户难以确认它
是否可选，右键时选区也可能丢失。本次保持内容不可编辑，同时使用文本光标和高对比选区，焦点不
阻断原生拖选，并增加 `Ctrl+C`、`Ctrl+A`、右键复制所选与全选对话。用户、Agent、系统提示和查询
摘要都在同一 transcript 中，因此不需要对每类消息另写复制逻辑。

### 38.4 技术难点与经验

**校验应分成“反序列化”和“依赖会话事实”两层。** Pydantic 适合验证单个 JSON 的字段形状，
但无法知道前一轮已验证页面。跨轮不变量应由持有 Session 的 Engine 校验，才能做安全恢复和审计。

**恢复必须复用原对象，而不是重新猜测。** 宿主没有用页面名、数据库表或文件名规则补造页面，
只复制先前已校验的 `LocatedPage`。这使恢复是确定性的状态连续性操作，不是新的业务判断。

**用户可复制与只读不冲突。** 禁止编辑是数据完整性约束，不应连带禁止选择、复制或审计。UI 控件
应分别处理 editability、selection、focus 和 clipboard，而不是把 disabled 当作全部交互关闭。

### 38.5 决策记录与验证

- ADR-114：单轮 JSON 形状由 Pydantic 校验，依赖历史状态的页面前置条件由 Engine 校验；
- ADR-115：只对同 cycle 已验证 `located_page` 做确定性字段恢复，并以 `decision_repaired` 留痕；
- ADR-116：新 cycle 清空页面绑定，历史消息仍可供模型重新核验，但不能直接成为宿主查询授权；
- ADR-117：桌面对话只读但始终可选择和复制，复制能力覆盖所有 transcript 文本。

定向测试覆盖二次业务查询漏页、最终完成漏页、无历史页面拒绝、恢复事件及页面上下文回灌；桌面
测试覆盖文本光标、选区配置、鼠标/键盘/右键绑定和全选行为。发布检查不执行数据库写入、MES代码
修改或真实业务接口调用。

---

## 39. 搜索策略自动纠正与选区层级修复（v0.7.8）

### 39.1 原始现象与日志证据

真实异常会话先完成两轮 `page_lookup`，从 Menu 候选取得
`Ckhy.MES.Client.CKClient.CustomYieldUpLoad`，再以精确文件名找到并读取
`zwqtmes/MESClient/CKClient/CustomYieldUpLoad.cs`。模型已经沿调用链识别
`btnSubmit_Click -> SendPostHttplvRequest -> SaveUploadLogs -> InsertYieldUploadLog`，随后发出：

```text
Grep pattern=InsertYieldUploadLog
path=zwqtmes
head_limit=40
glob/type=<missing>
```

该调用有明确符号和输出上限，但目录级 Grep 缺少文件类型过滤器，符合既有机械阻断条件。Event
顺序表明错误发生在源码追踪阶段：页面数据库查询已经成功，业务数据查询尚未开始；不是 Menu 或
SQL Server 再次出错。旧 Runtime 杀掉越界进程后只抛普通 `ClaudeCodeError`，Engine 无法区分
“可以要求模型补一个参数”和“不可恢复的 Runtime 失败”，所以整个任务直接进入 failed。

同一版本还暴露 UI 层级问题：鼠标选择“Agent · 回应”标题可以看到颜色，正文虽然存在 selection，
但 `assistant_message/user_message/system_message` 的卡片背景标签优先级高于 Tk 特殊 `sel` 标签，
视觉上像没有选中。复制功能本身存在，缺陷是正文选区被背景绘制覆盖。

### 39.2 方案选择

没有放宽目录 Grep 规则让它扫描项目内所有类型，也没有继续把一次参数遗漏升级成用户可见的终态
失败。Runtime 现在把搜索阻断映射为带结构化字段的 `RuntimePolicyBlockedError`：

- `policy/operation/reason` 只含脱敏审计摘要；
- `retryable=true` 仅用于缺少 `glob/type/head_limit` 和可缩窄的宽 Glob；
- 路径越界、搜索次数超限等边界不可自动重试；
- 开发和异常的 inspect 流程最多纠正一次，implement/verify 不自动重试；
- 被阻断调用的结果不进入 ACE，失败 Run、`policy_blocked` 和 `policy_repair_requested` 全部保留；
- 修正 Prompt 要求复用已验证页面/文件、补语言过滤器并保持相同调查目标，不能要求用户执行搜索。

如果修正轮次再次违反策略，任务仍然失败。这样既避免“一次漏参就终止”，也不允许无限重试绕过
单轮搜索预算。对话选区则采用最小 UI 修复：所有消息标签创建完成后提升 `sel` 优先级，不改变
消息内容、只读状态或复制实现。

### 39.3 核心技术与边界

一次策略阻断对应一个失败的 Runtime Run，而不是伪装成成功；任务 aggregate 仍保持 inspecting，
随后用同一 `runtime_session_id` 开启纠正 Run。这个区别对恢复和审计很重要：可以解释第一次调用
为什么没有结果，也能证明最终证据来自修正后的合规调用。

自动纠正只发生在只读 inspect。实现或验证阶段可能已经修改文件或运行命令，中断后不能简单重试，
仍沿用 recovery_required 处理副作用不确定性。搜索边界本身没有变化：目录 Grep 仍要求过滤器和
1..100 输出上限，Glob 仍禁止全项目枚举，显式路径仍必须位于授权根目录。

### 39.4 决策记录与验证

- ADR-118：搜索策略错误通过 Runtime 端口的结构化异常传递，不让 Engine 解析错误文案；
- ADR-119：只对可纠正的 inspect 搜索阻断自动重试一次，越界、预算和第二次违规保持终止；
- ADR-120：被阻断 Run 和自动修正分别留痕，不把策略纠正伪装成同一次成功执行；
- ADR-121：只读 transcript 的 `sel` 标签必须高于所有带背景的消息标签。

测试覆盖 Runtime 异常类型、可纠正/不可纠正分类、异常与开发流程的一次自动修正、第二次阻断终止、
审计事件、修正 Prompt，以及标题/正文选区标签优先级。没有执行 MES 代码修改、数据库写入或业务
接口调用。

---

## 40. 真实异常流程验收与纠正预算收敛（v0.7.9）

### 40.1 真实验收场景

使用“**小米良率上传页面，上传数据后没有保存日志；本地正常，发布后产线上传成功但日志没有
记录**”作为完整异常输入，在 `生物` 项目知识下对独立 MES 测试工作区执行真实异常流程。测试仅
授权页面查询、源码读取/搜索和 SQL Server `SELECT`，没有授权代码修改、数据库写入或外部上传。

会话 `38e7af0e-c777-45d8-bb80-ad9c78493b44` 最终进入 `completed`。审计记录包含 6 次 Runtime
Run、17 组只读工具开始/结束事件、4 个数据库查询批次和 1 次搜索策略纠正。流程先以页面标题精确
查询 Menu，空结果后做模糊查询；它识别 Menu 返回的 `FCModelUpload` 是良率机型配置页而不是问题
页面，再通过限定文件名和调用链确认真正页面为 `CustomYieldUpLoad.cs`。随后读取服务、实体和
EF 上下文，并由宿主执行表结构、最近日志和应用错误日志查询，最终生成异常能力文档。

### 40.2 诊断结果与证据边界

已确认的机制根因是日志保存与上传成功提示彼此独立：`SaveUploadLogs` 捕获并吞掉日志写库异常，
而界面只根据外部上传结果 `OK` 显示成功，因此生产写库失败会表现为“上传成功但没有日志”。数据库
证据还显示 `YIELDUPLOAD_LOG.RESULT` 实际为 `nvarchar(200)`，但实体使用 `MaxLength(50)`，代码
又按 200 截断；该漂移是长返回内容触发 EF 校验失败的高风险候选。实际连接库最近仍有成功日志，
所以不能把“表完全不存在”当作已证实根因。

最终置信度保持 `0.6`：代码机制和结构漂移有直接证据，但生产环境触发异常没有日志，因为异常本身
被吞掉。下一步应先记录真实异常、对齐实体与数据库字段，并核对产线连接目标，不能在证据不足时
宣称已经唯一定位生产原因。

### 40.3 从首次失败得到的 Runtime 经验

首次真实跑测在较早源码阶段成功纠正一次缺少 `head_limit` 的 Grep，完成业务数据查询后又遇到一个
独立的目录 Grep 缺少 `glob/type`。v0.7.8 把“一次纠正”按整个用户命令计数，导致第二个独立阶段
无法纠正并提前失败。这个问题不是安全策略太严，而是预算生命周期过宽。

v0.7.9 将计数改为 `consecutive_search_repair_rounds`：一次通过校验的结构化模型决策会结束当前
违规链并归零；后续独立阶段可以获得自己的一次有界修正。连续第二次违规仍失败，路径越界和搜索
预算超限仍不可纠正。开发与异常 Engine 同步采用该语义，并用单元测试覆盖“阻断 -> 成功决策 ->
独立阻断 -> 成功完成”以及原有的连续二次阻断场景。

### 40.4 决策记录

- ADR-122：搜索纠正预算属于连续违规链，不属于整个多阶段命令；
- ADR-123：只有通过 Runtime 与结构化契约校验的决策才能重置纠正链；
- ADR-124：真实异常验收必须同时核对状态、事件、查询审计和最终能力文档；
- ADR-125：只读验收不能用“Git 工作区干净”替代权限证据，测试仓库可能原本已有用户改动。

---

## 41. 继续对话附件重封装与结论交付（v0.7.10）

### 41.1 现象与直接证据

用户在异常会话继续对话时粘贴截图，桌面提示
`Incident attachment size changed or is invalid: incident-screenshot.png`。该异常来自
`IncidentEngine._validate_attachments`，发生在 Runtime 启动之前，说明不是 Claude、页面定位或
SQL 查询失败。现场隔离文件仍可被 Pillow 完整解析为 PNG，实际大小为 35003 字节；用陈旧的
`size_bytes` 构造同一附件可以稳定复现原错误。

应用日志没有记录这次桌面后台异常，也暴露了组件边界的证据缺口：用户能看到弹窗，但后续无法从
本地日志还原发生在哪个流程和会话。

### 41.2 方案与安全权衡

不能简单删除大小一致性检查，否则目录外文件、损坏图片或发送前替换内容都可能直接进入模型。
本次选择在桌面附件边界“重新封装”而不是放宽核心：

1. 路径必须严格属于 ACE 隔离附件根目录，并与附件 ID 和固定文件名匹配；
2. 重新检查文件存在、10 MiB 上限和 4000 万像素上限；
3. 用 Pillow 验证内容确实是可解码 PNG；
4. 图片有效时刷新实际大小，再交给仍保持严格校验的 Engine；
5. 文件损坏、越界或不属于隔离目录时保留待发状态并要求用户重新粘贴。

这样可修复剪贴板或图片提供方产生的无害元数据漂移，同时不把任意路径或任意内容当作可信附件。
桌面后台操作失败和附件准备失败新增最小结构化日志，只记录流程、会话和安全错误摘要。

### 41.3 最终异常结论契约

原模型契约已经包含 `diagnosis` 和 `recommended_actions`，桌面也会显示，但最终 Assistant 正文只
保存 `message`，不同模型可能只给一句“诊断完成”，导致结论和解决方案不够醒目。v0.7.10 不把
原因写成宿主规则，而是继续让模型基于证据判断；宿主只固定交付结构：

- `结论`：一句话回答最终判断；
- `为什么出现这个异常`：展示模型的因果诊断，并要求标注确定性等级；
- `解决方法`：展示具体修复或安全验证步骤；
- `结论置信度`：明确当前证据强度。

Prompt 明确要求完成决策提供因果链和至少一个动作。为兼容历史会话，数据模型不拒绝旧记录中为空
的 `recommended_actions`；若新模型仍漏填，宿主显示“继续补充证据”的保守动作，而不是虚构解决
方案。API 的 `IncidentOutcome.message`、持久化对话和桌面显示使用同一格式，原结构化字段继续供
能力沉淀和其他客户端消费。

### 41.4 决策记录

- ADR-126：附件元数据漂移在受控 Store 边界重封装，核心 Engine 不取消严格校验；
- ADR-127：有效内容可以刷新大小元数据，损坏和目录越界仍必须拒绝；
- ADR-128：异常原因由模型判断，最终展示结构由宿主保证；
- ADR-129：历史完成会话保持可读取，缺少解决动作时使用补证据提示而非加载失败；
- ADR-130：桌面后台异常必须写入既有本地日志，补齐弹窗之外的追溯证据。

---

## 42. Token 效率与能力保持优化（v0.7.11）

### 42.1 测量结论

真实“小米良率上传日志缺失”异常会话基线共进行 6 次 Runtime、4 批 SQL 和 19 个模型回合；
输入 41,448、输出 25,768、缓存读取 307,072，总计 374,288 Token，耗时约 249 秒，模型费用约
1.02 美元。问题不只是最终答案长，而是完整数据库目录、工作流、RAG/Hermes 上下文和工具历史在
多轮中反复出现。

第一次尝试只缩短续聊 Prompt、仍使用 `--resume`，一个简单追问仍输入 47,511 Token。原因是
Claude Code 会恢复整个既有工具会话，宿主的新短 Prompt 无法消除 Runtime 自身的历史。这证明
Token 优化必须改变上下文路由，而不能只改提示词措辞。

### 42.2 已采用设计

完成后的无附件追问先交给一个全新、无工具、无历史的紧凑模型会话。它只看到上个 cycle 的有界
证据摘要，并判断“已有证据可回答”或“需要完整调查”。简单解释直接完成；新症状、新环境、事实
核验或图片会升级到原完整会话，恢复已定位页面、源码工具、RAG、数据库和事件/Recovery 内核。

开发与异常流程统一取消每轮全量 `describe_schema()` 注入。数据库能力没有删除：模型从源码识别
候选表，必要时用同一批参数化只读 SQL 查询元数据和业务证据。Prompt 要求把已知、可共同解释的
小查询合并，避免每条 SQL 单独消耗一次模型往返。完整调查的稳定 Prompt 内容保持顺序不变，把
动态阶段说明放到尾部，以继续利用提供方前缀缓存。

### 42.3 真实效果

紧凑续聊实测为 1 次 Runtime、0 次 SQL、2,716 输入与 543 输出 Token，耗时约 5.2 秒、费用约
0.027 美元。重新执行完整异常调查后仍完成 Menu 页面定位、限定源码调用链、SQL Server 只读取证
和原因/解决方案输出；Runtime 从 6 降到 4、SQL 批次从 4 降到 2、模型回合从 19 降到 11。

完整调查总 Token（含缓存）从 374,288 降到 169,739，下降约 54.6%；直接输入加输出从 67,216
降到 54,411，下降约 19%；费用从约 1.02 降到 0.68 美元，下降约 33.6%；耗时从约 249 秒降到
150 秒，下降约 39.9%。这些数字来自单次真实场景，受模型响应和缓存状态影响，不应作为固定 SLA。

### 42.4 能力与安全边界

- 条件不足仍由模型追问，页面映射优先和有界 Glob/Grep 策略不变；
- 深度流程仍可读相关源码、执行参数化只读 SQL、调用 RAG/Hermes 并输出完整因果结论；
- 简单续聊不携带原始数据库行，最多携带十条查询审计摘要；
- 新图片直接走完整调查，避免紧凑路由遗漏视觉证据；
- `answer` 必须有诊断且复用已验证页面，`investigate` 不覆盖原 Runtime session；
- SQL 仍受只读、参数、超时、行数、敏感列和轮次限制，优化不扩大权限。

### 42.5 决策记录

- ADR-131：完成后追问先由独立无工具紧凑会话路由，不恢复完整工具历史；
- ADR-132：紧凑路由只做证据内回答或升级，新的事实调查必须回到完整流程；
- ADR-133：数据库目录改为源码驱动的按需元数据查询，不在每轮注入全量结构；
- ADR-134：可共同解释的小查询优先同批执行，宿主原查询安全策略保持不变；
- ADR-135：Prompt 稳定前缀是缓存契约，不能为追求字符更少而在每轮改变前缀内容。

---

## 43. Engine 与 SQLite 基础能力收敛（v0.7.12）

### 43.1 问题判断

项目的主要臃肿不是大量无用文件，而是相同的机械职责散落在两个领域实现中。重构前，开发与异常
Engine 都自行创建 Runtime Run、记录活动、更新 heartbeat、投影 UI 进度、终结 Run、合并 Usage
并处理 RAG 成败事件；两个 SQLite Store 也各自维护连接 PRAGMA、Event sequence、Run 终态、命令
回执和状态回放。领域逻辑继续增长时，这些副本很容易出现安全约束和审计字段不一致。

静态清点只发现两个很小的端口文件没有直接生产引用，CLI、Streamlit、JSON Store 等看似可选的
模块仍承担兼容、备用入口或测试替身职责。本轮没有为了降低文件数进行批量删除，也没有把开发与
异常 Engine 合并为一个依靠条件分支的万能类。

### 43.2 采用的结构

新增三个小型组合组件：

- `RuntimeLifecycle`：只处理 Run 开始、可观测活动、测试命令审计、heartbeat、进度投影和终态；
- `retrieve_knowledge_context()`：统一有界知识检索、引用事件和失败降级；
- `SQLiteRuntimeDatabase`：统一连接、UUID、JSON 记录、Event/Run/Command 不可变规则和状态回放。

`AgentEngine` 继续拥有审批、实施、验证、Artifact 和 Recovery；`IncidentEngine` 继续拥有页面映射、
截图、查询阶段、SQL 纠错和异常结论。两个 Store 继续拥有各自 snapshot/schema/旧 JSON 迁移，
`SQLiteTaskStore` 还独占 Decision 与 Artifact metadata。共享类通过组合注入，不建立 Engine 继承树。

主流程注释按“进入授权状态—准备证据—打开 Runtime—校验决定—调用受控服务—持久化结果”编号，
说明操作意图和边界。注释没有逐行翻译代码，也没有把 Prompt 业务规则复制进实现，避免文档再次
成为难以同步的第二份流程代码。

### 43.3 技术难点与解决方案

**抽取不能破坏事务原子性。** SQLite 共用类接收现有 connection 执行追加操作，事务的
`BEGIN IMMEDIATE`、revision 比较、snapshot 更新和 commit 仍由领域 Store 控制。因此 Event、
领域记录、Run、Command Receipt 与 snapshot 仍在同一事务中成功或回滚。

**既有两套表结构并不完全相同。** 开发表保存更丰富的 Event/Run/Command 索引列，异常表保持较精简
的历史结构。本轮使用经过正则校验、只由代码构造的 `SQLiteRuntimeLayout` 选择列布局，复用规则但
不动态接受用户表名，也不强迫已有用户数据迁表。

**失败类型属于领域边界。** 共用 Runtime 生命周期接收异常工厂：开发流程仍可抛出
`PolicyViolation`，异常流程使用普通契约错误。共享组件不决定任务应失败、修复还是恢复，这些选择
仍由对应 Engine 依据当前模式和副作用风险完成。

**抽取后可读性不能下降。** Engine 保留一眼可见的主循环和领域分支，公共组件名称直接表达动作；
类级流程说明和关键阶段注释解释“为什么”，细节实现留在方法内部。重构后四个原重复文件从约
4580 行降到约 3880 行；新增共享实现约 756 行，净行数不是主要指标，关键收益是约束只有一个实现
来源，且新增直接测试覆盖共享边界。

### 43.4 决策记录与验证

- ADR-136：共享机械能力通过组合组件复用，不用 Engine 继承合并不同领域流程；
- ADR-137：SQLite 共享连接与不可变生命周期规则，领域 Store 保留 snapshot 和事务所有权；
- ADR-138：既有开发/异常表结构通过受信任 Layout 兼容，本轮不做生产数据迁移；
- ADR-139：主流程注释解释阶段、意图和安全边界，不逐行复述实现；
- ADR-140：可选入口与兼容存储先保留，删除文件必须另有使用证据和产品决定。

定向测试覆盖 Runtime 活动/测试/终态事件、Usage 合并、双 Store 共用数据库和统一 PRAGMA，并由
全部非 live 测试覆盖开发审批、异常调查、持久化、恢复、RAG、Hermes、UI 与兼容路径。本轮不执行
真实模型调用、SQL Server 查询、数据库写入或目标项目代码修改。
...
v0.7.12 虽然为两个 Engine 增加了编号式主流程说明，但新增注释和 Docstring 主要使用英文，无法
达到“让项目维护者快速读懂流程”的目标。代码可运行不等于代码容易接手；对于以中文为主要维护
语言的项目，解释状态、证据边界和恢复原因的注释也应使用中文。

### 44.2 采用的规则

- Engine 类说明使用中文列出完整主流程；
- 关键循环按“第 1 步”到“第 8 步”说明当前操作、目的和安全边界；
- Runtime 生命周期、RAG 检索、SQLite 基础设施及领域 Store 的模块/类/方法说明统一为中文；
- 注释重点解释为什么需要持久化、为什么只能只读、为什么需要恢复，不逐行翻译代码；
- 类名、字段、EventType、Prompt、错误契约和第三方接口参数保持原有英文，避免破坏协议兼容；
- SQL 字符串和模型系统提示词不作为维护注释翻译，除非另有产品语言要求。

### 44.3 决策记录

- ADR-141：面向项目维护者的新增代码注释和 Docstring 默认使用中文；
- ADR-142：代码标识符与运行契约保持英文，注释语言调整不得改变程序行为；
- ADR-143：注释描述职责、阶段和原因，不复制实现细节或模型提示词全文。

本次只调整注释、Docstring、版本与文档，不修改 Runtime、状态机、SQLite Schema、SQL 权限或模型
行为；仍使用全部非 live 测试确认代码解析和现有流程没有变化。

---

## 45. 自适应行动与证据完整性规划（v0.7.14）

### 45.1 要解决的矛盾

异常和开发任务不能依赖一个完全固定的步骤表：用户已经给出精确路径时不应再次查询页面菜单，
代码机制足够清楚时也不必强制查询业务数据。但如果把下一步全部交给模型且没有宿主校验，模型又
可能漏看截图、漏查记录、忽略冲突或在证据不足时提前完成。

计划采用混合架构：模型根据当前事实选择下一项最有价值的行动；宿主不规定取得证据的顺序，只
固定权限、预算、证据来源、最终覆盖类别和 Claim 引用完整性。状态机继续负责生命周期与恢复，
Evidence Gate 负责检查最终结果是否可靠，两者都不替模型判断业务语义。

### 45.2 关键设计

- EvidenceItem 只能从用户消息、附件、真实工具活动、QueryObservation、Artifact 和测试事实投影；
- 模型输出 Claim 并引用 Evidence ID，模型推理本身不能成为证据；
- 异常完成至少覆盖问题上下文、目标身份、当前行为、因果链、解决方法和证据缺口；
- 数据库、截图、外部接口和验证属于条件证据，由任务事实触发，不要求每次都执行；
- 不使用总分阈值，按证据类别、引用、冲突和当前权限检查；
- 先旁路 Ledger，再 shadow Evidence Gate，校准后先在异常流程启用，最后接入开发流程；
- SQL 只读、源码搜索边界、审批、预算、Event Store 和 Recovery 不变。

完整模型、阶段、验收场景、迁移和风险记录在
`docs/tasks/ACE-RUNTIME-026-adaptive-evidence-loop.md`。当前版本没有新增 Evidence API，也没有改变
两个 Engine 的执行顺序；这是后续迭代的设计基线，不应在产品介绍中写成已经实现。

### 45.3 决策记录

- ADR-144：状态机固定生命周期，Evidence Gate 固定结果完整性，模型选择调查顺序；
- ADR-145：宿主事实生成 Evidence，模型只能建立 Claim 与引用关系；
- ADR-146：证据完整性采用分领域类别和条件要求，不采用统一数值评分；
- ADR-147：先 shadow 后 enforcement，先异常后开发，每个 Phase 独立发布并可回退；
- ADR-148：灵活性不能扩大工具权限、数据库能力或自动重试预算。
